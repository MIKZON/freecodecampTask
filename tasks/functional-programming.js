...
function getRating(watchList) {
  let averageRatingMap = watchList.map(({ Director: dir, imdbRating: rate }) => ({dir, rate}))
                                  .filter((el) => el.dir === 'Christopher Nolan');
  let averageRatingMapReduce = averageRatingMap.reduce((obj, rating) =>
                                   obj + parseFloat(rating.rate)  , 0 )
  return averageRatingMapReduce / averageRatingMap.length;
}
...
const squareList = arr => {
  ...
  const filtered = mapped.filter((el) => el > 0 && Number.isInteger(el));
